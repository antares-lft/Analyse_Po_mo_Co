...
# Calcul des moyennes et IC 95% par espèce
t_value <- 1.03  # t de Student à 95% pour df = 5
...
# Conversion des colonnes nécessaires
donnees <- donnees %>%
  mutate(across(c(dN_dS, Pis), as.numeric))  # Pis aussi converti

# Filtrer les données valides (Pis > 0 et non NA)
donnees_filtered <- donnees %>%
  filter(!is.na(Pis), Pis > 0, !is.na(dN_dS))

# Calcul des moyennes et IC 75% par espèce et Pis
t_value <- qt(1 - 0.125, df = 5)  # t de Student à 75% IC, df=5 (~1.15)

resume_stats <- donnees_filtered %>%
  group_by(species, Pis) %>%
  summarise(
    moy_dN_dS = mean(dN_dS, na.rm = TRUE),
    sd_dN_dS = sd(dN_dS, na.rm = TRUE),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
    se = sd_dN_dS / sqrt(n),
    ic75_low = moy_dN_dS - t_value * se,
    ic75_high = moy_dN_dS + t_value * se
  )

# Régression linéaire globale (toutes espèces confondues)
modele_lm <- lm(dN_dS ~ Pis, data = donnees_filtered)
summary_lm <- summary(modele_lm)

# Extraire coefficients
intercept <- round(coef(modele_lm)[1], 3)
slope <- round(coef(modele_lm)[2], 3)

# Extraire p-value pour le slope
pval <- summary_lm$coefficients[2,4]
pval_text <- ifelse(pval < 0.001, "p < 0.001", paste0("p = ", signif(pval, 3)))

# Construire le texte à afficher
equation_text <- paste0("y = ", slope, " * x + ", intercept, "\n", pval_text)

# Trouver les coordonnées pour le texte (en haut à droite du graphique)
x_pos <- max(donnees_filtered$Pis, na.rm = TRUE)
y_pos <- max(donnees_filtered$pN_pS, na.rm = TRUE)

# Graphique avec annotation
ggplot() +
  geom_point(data = donnees_filtered, aes(x = Pis, y = dN_dS), 
             color = "orange", alpha = 0.6, size = 2) +
  geom_point(data = resume_stats, aes(x = Pis, y = moy_dN_dS), 
             shape = 15, color = "blue", size = 3.5) +
  geom_errorbar(data = resume_stats,
                aes(x = Pis, ymin = ic75_low, ymax = ic75_high),
                width = 0.01, color = "blue") +
  geom_smooth(data = donnees_filtered, aes(x = Pis, y = dN_dS), 
              method = "lm", se = FALSE, color = "red", linewidth = 1) +
  scale_x_log10() +
  theme_minimal() +
  labs(
    title = "Mean and individuals dN/dS as a function of pS (IC 75%) with linear regression",
    x = "pS",
    y = "dN/dS"
  ) +
  annotate("text", x = x_pos, y = y_pos, label = equation_text, 
