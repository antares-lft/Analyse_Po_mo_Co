...
ggplot(donnees_par_espece, aes(x = taille_pop, y = N_effectif)) +
  ...
  scale_color_gradient(name = "log10(Branch length)", low = "blue", high = "yellow") +
  
  # Annotations à GAUCHE avec des coordonnées fixes dans la zone visible
  ...
  annotate("text", x = lims[1] + 0.05 * diff(lims),
           ...
           label = paste0("Slope (lm) = ", pente_lm),
           hjust = 0, size = 4.5) +
