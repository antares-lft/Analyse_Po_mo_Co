...
ggsave("/home/alafitte/Internship/Rapport de stage/Images/Results/Intermediate_recombination/Recent/Picture 1.png",width = 8, height = 6, dpi = 300)
```
...
# === Données théoriques de Julia ===
points_julia <- tibble(
  taille_pop = c(100, 250, 500, 1000, 2500, 5000),
  dnds = c(0.641439, 0.61255, 0.509405, 0.236966, 0.00802157, 3.88478e-05)
) %>%
  mutate(
    log10_taille_pop = log10(taille_pop)
  )

# Régression linéaire Julia
modele_julia <- lm(dnds ~ log10_taille_pop, data = points_julia)
resume_julia <- summary(modele_julia)
r2_julia <- round(resume_julia$r.squared, 3)

# Régression linéaire simulation
modele <- lm(moyenne_points ~ log10_taille_pop, data = donnees_par_espece)
resume_modele <- summary(modele)
r2 <- round(resume_modele$r.squared, 3)
pval <- signif(coef(resume_modele)[2, 4], 3)

# === Comparaison directe Julia vs simulation ===
tailles_julia <- c(100, 250, 500, 1000, 2500, 5000)

empirique_moyenne <- donnees_par_espece %>%
  filter(taille_pop %in% tailles_julia) %>%
  group_by(taille_pop) %>%
  summarise(moyenne_sim = mean(moyenne_points), .groups = "drop") %>%
  arrange(taille_pop)

comparaison <- tibble(
  taille_pop = tailles_julia,
  dnds_julia = c(0.641439, 0.61255, 0.509405, 0.236966, 0.00802157, 3.88478e-05)
) %>%
  left_join(empirique_moyenne, by = "taille_pop") %>%
  drop_na()

r2_comparaison <- round(cor(comparaison$dnds_julia, comparaison$moyenne_sim)^2, 3)

# === Graphique final ===
ggplot() +
  # Points empiriques (simulations)
  geom_point(data = donnees_par_espece,
             aes(x = log10_taille_pop, y = moyenne_points, color = log10_branch_length), size = 3) +
  scale_color_viridis_c(option = "plasma", name = "Branch length (log10)") +

  # Régression simulations
  geom_smooth(data = donnees_par_espece,
              aes(x = log10_taille_pop, y = moyenne_points),
              method = "lm", se = TRUE, color = "black", fill = "lightgray") +

  # Points Julia
  geom_point(data = points_julia,
             aes(x = log10_taille_pop, y = dnds),
             shape = 17, size = 3.5, color = "blue", fill = "blue") +

  # Régression Julia
  geom_smooth(data = points_julia,
              aes(x = log10_taille_pop, y = dnds),
  ...
  annotate("text",
           x = max(donnees_par_espece$log10_taille_pop),
           y = max(donnees_par_espece$moyenne_points),
           label = paste0("R² simulation = ", r2), hjust = 1, size = 4.5) +
  annotate("text",
           x = max(donnees_par_espece$log10_taille_pop),
           y = max(donnees_par_espece$moyenne_points) - 0.1,
           label = paste0("p = ", pval), hjust = 1, size = 4.5) +
  annotate("text",
           x = min(points_julia$log10_taille_pop),
           y = max(points_julia$dnds),
           label = paste0("R² Julia = ", r2_julia), hjust = 0, size = 4.5, color = "blue") +
  annotate("text",
           x = min(donnees_par_espece$log10_taille_pop),
           y = min(donnees_par_espece$moyenne_points),
           label = paste0("R² Julia vs empirical = ", r2_comparaison),
           hjust = 0, size = 4.5, color = "darkgreen") +
...
ggsave("/home/alafitte/Internship/Rapport de stage/Images/Results/Intermediate_recombination/Recent/Picture 2.png",width = 8, height = 6, dpi = 300)
```
```{r cars}
library(tidyverse)
